...
RUN CGO_ENABLED=0 \
    cd git-init && go build -o /tmp/tektoncd-catalog-git-clone && cd -

COPY source-tool source-tool
RUN CGO_ENABLED=0 \
    cd source-tool && go build -o /tmp/slsa-source-tool && cd -
...
COPY --from=builder /tmp/slsa-source-tool /usr/bin/slsa-source-tool
